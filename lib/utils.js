...
exports.utils = {
    ...
    */
    ...
    /**
    * Calculates the amount of time remaining until the next
    * notification timer
    * @param {int} lastCTATime - the lastSidebarCTACompleteTime
    * @returns The next CTA complete time
    */
    getNextTimerCompleteTime: function(lastCTATime) {
        // get the time elapsed since the last cta completion
        let timeElapsed = parseInt(module.exports.utils.getTimeElapsed(lastCTATime));
        // get the amount we need to add to make up for the difference between
        // CTA completion and when the timer should be done
        let timeElapsedDiff = parseInt((intervals.defaultSidebarInterval - timeElapsed > 0 ? intervals.defaultSidebarInterval - timeElapsed : 0));

        return timeElapsedDiff + timeElapsed;
    },
    /**
    ...
    */
    getTimeElapsed: function(sidebarLaunchTime) {
        ...
        var timeElapsed = (Date.now() - lastSidebarLaunch.getTime()) / intervals.timeElapsedFormula;

        // to avoid ambiguity between various implementations, we are using toFixed to
        // ensure that return value, if a decimal, never has more than one digit after
        // the decimal place.
        if (!Number.isInteger(timeElapsed)) {

...
onSidebarEvent('action', function(action) {
    switch (action) {
        case 'hideSidebar':
            ...
            // hide any active UITour highlights
            sidebarUtils.removeHighlight();
            break;
        case 'incrementStep':
...
onSidebarEvent('showSidebar', function() {
    module.exports.sidebarManager.toggleSidebar();
});
...
exports.sidebarManager = {
    ...
    setSidebarProps: function() {
        ...
        if (currentStep === 4) {
            sidebarProps = module.exports.sidebarManager.sidebarKeys[track][3];
        } else {
    ...
    intentHandler: function(intent) {
        switch (intent) {
            case 'internetHealth':
                tabs.open('https://www.mozilla.org/internet-health/');
                break;
            case 'search':
            ...
            case 'privateBrowsing':
                ...
                break;
            case 'privateBrowsingWeb':
                tabs.open('https://www.mozilla.org/firefox/private-browsing/');
                break;
            case 'tryAddons':
                tabs.open('https://addons.mozilla.org/');
                break;
            default:
                break;
        }
    ...
    */
    toggleSidebar: function() {
        ...
        if (step !== 4 && typeof tokensArray !== 'undefined'
            && utils.getTimeElapsed(lastCTACompleteTime) >= intervals.defaultSidebarInterval
            && tokensArray.indexOf('token' + step) > -1) {
            module.exports.sidebarManager.showSidebar(sidebarProps);
        } else {
            if (typeof isSidebarVisible !== 'undefined' && isSidebarVisible === true) {

...
exports.sidebarManager = {
     ...
     */
    attachCommonSidebarListeners: function(worker) {
        ...
        worker.port.on('loadSidebar', function(props) {
            ...
            module.exports.sidebarManager.onDemandSidebar(props);
        });
    },
    /**
    ...
    setSidebarProps: function() {
        var latestToken = 'token' + storageManager.get('step');
        var sidebarProps = {};
        ...
        // reset our assigned token flag for the new sidebar, because it would restart the timer on every
        // content5 sidebar open if we didn't and get the sidebar props
        if (storageManager.get('step') === 5) {
            sidebarProps = module.exports.sidebarManager.sidebarKeys[track][4];
        } else {
            // in every other case, we get the properties before we increment the contentStep as
            // arrays are 0 indexed.
            sidebarProps = module.exports.sidebarManager.sidebarKeys[track][storageManager.get('step') || 0];
            tokensManager.assignedToken = false;
        }

        // determine the current content step we are on
        if (tokens.indexOf(latestToken) > -1 && utils.getTimeElapsed(storageManager.get('lastSidebarCTACompleteTime')) >= intervals.defaultSidebarInterval
            && storageManager.get('step') !== 5) {
            contentStep = typeof storageManager.get('step') !== 'undefined' ? (storageManager.get('step') + 1) : 1;
        } else {
            contentStep = typeof storageManager.get('step') !== 'undefined' ? (storageManager.get('step')) : 1;
        }
     ...
     */
    onDemandSidebar: function(sidebarProps) {
        var sidebarIdTitle = module.exports.sidebarManager.sidebarKeys[sidebarProps.track][sidebarProps.step - 1];
        ...
        module.exports.sidebarManager.content = sidebar.Sidebar({
            ...
            onAttach: function(worker) {
                ...
                module.exports.sidebarManager.attachCommonSidebarListeners(worker);
                ...
                worker.port.on('cta_complete', function() {
                    ...
                    timers.clearInterval(scheduler.destroyTimer);
                    // start a new 3 week destroy timer
                    scheduler.startDestroyTimer(intervals.nonuseDestroyTime);
                });

                // if we're notified that the user selected the "next" button
    ...
    */
    showSidebar: function(sidebarProps) {
        ...
        module.exports.sidebarManager.content = sidebar.Sidebar({
            ...
            onAttach: function(worker) {
                ...
                worker.port.on('cta_complete', function() {
                    ...
                    if(!tokensManager.assignedToken) {
                        ...
                        scheduler.autoCloseTimer(intervals.afterInteractionCloseTime);
                    }
                });
        ...
        aboutHome.modifyAboutHome(sidebarProps.track, sidebarProps.step);
    },
    /**
    ...
    */
    toggleSidebar: function() {
        ...
        if (storageManager.get('step') !== 5 && typeof storageManager.get('tokens') !== 'undefined'
            ...
            if (storageManager.get('step') === 5 && storageManager.get('canShowReward')) {
                if (storageManager.get('isSidebarVisible') === true) {
                    module.exports.sidebarManager.content.hide();
                }
                else {
    ...
    loadNext: function() {
        // get the time elapsed since the last cta completion
        var timeElapsed = utils.getTimeElapsed(storageManager.get('lastSidebarCTACompleteTime'));

        // get the amount we need to add to make up for the difference between CTA completion and when the timer should be done
        var timeElapsedDiff = (intervals.defaultSidebarInterval - utils.getTimeElapsed(storageManager.get('lastSidebarCTACompleteTime')) > 0 ? intervals.defaultSidebarInterval - utils.getTimeElapsed(storageManager.get('lastSidebarCTACompleteTime')) : 0);

        // fast forward the lastSidebarCTACompleteTime to complete now
        storageManager.set('lastSidebarCTACompleteTime', timeElapsedDiff + timeElapsed);

        // set the sidebar props so we can advance sidebars
        module.exports.sidebarManager.setSidebarProps();

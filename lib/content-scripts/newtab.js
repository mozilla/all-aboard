...
exports.newtab = {
        ...
        this.aboutNewtab = pageMod.PageMod({
            ...
            onAttach: function(worker) {
                ...
                try {
                    ...
                    if (!canUndo) {
                        // emit modify event and passes templates as an object
                        worker.port.emit('modify', module.exports.newtab.getTemplates(), false);
                    }
                    // if we can undo the auto import, send the footercontent to our sidebar, and then listen for when we would like to actually execute the "undo"
                        ...
                        // emit modify event and passes templates as an object
                        worker.port.emit('modify', module.exports.newtab.getTemplates(), true);
                ...
                worker.port.on('pageModified', function() {
                    ...
                    module.exports.newtab.resetPreload();
                });
    ...
    /**
     * Sets localised template strings and returns the modified template(s)
     */
    getTemplates: function() {
        const footerContentURL = './tmpl/about-newtab-footer.html';
        const headerContentURL = './tmpl/about-newtab-header.html';
        let footerContent = self.data.load(footerContentURL);
        let headerContent = self.data.load(headerContentURL);

        headerContent = headerContent.replace('%url', self.data.url('media/moving-truck.png'));

        return {
            header: utils.processTmpl(headerContent),
            footer: utils.processTmpl(footerContent)
        };
    }
};

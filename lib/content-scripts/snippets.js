...
exports.snippets = {
    replaceSnippetCopy: function(track, contentURL, imageURL) {
        var snippetContent;
        var sidebarProps = storageManager.get('sidebarProps');
        ...
        switch(copyNumber) {
            case 1:
                ...
                snippetContent = self.data.load(contentURL).replace('%url', self.data.url(imageURL)).replace(
                    '%snippetCopy',
                    SNIPPET_COPY[track][sidebarProps.step - 1].copy1
                );
                break;
            case 2:
                // load snippet HTML
                snippetContent = self.data.load(contentURL).replace('%url', self.data.url(imageURL)).replace(
                    '%snippetCopy',
                    SNIPPET_COPY[track][sidebarProps.step - 1].copy2
                );
                break;
            case 3:
                // load snippet HTML
                snippetContent = self.data.load(contentURL).replace('%url', self.data.url(imageURL)).replace(
                    '%snippetCopy',
                    SNIPPET_COPY[track][sidebarProps.step - 1].copy3
                );
                break;
            default:
                // load snippet HTML
                snippetContent = self.data.load(contentURL).replace('%url', self.data.url(imageURL)).replace(
                    '%snippetCopy',
                    SNIPPET_COPY[track][sidebarProps.step - 1]
                );
                break;
        }
